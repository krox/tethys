...
func Load(path string) (*Book, error) {
	...
	if len(data)%16 != 0 {
		return nil, errors.New("invalid polyglot book length")
	}
	entries := make([]entry, 0, len(data)/16)
	for i := 0; i < len(data); i += 16 {
		...
		entries = append(entries, entry{key: key, move: move, weight: weight})
	}
...
func decodeMove(m uint16) string {
	toFile := int(m & 0x7)
	toRank := int((m >> 3) & 0x7)
	fromFile := int((m >> 6) & 0x7)
	fromRank := int((m >> 9) & 0x7)
	promo := int((m >> 12) & 0x7)

	// Polyglot encodes castling as king-to-rook squares.
	if fromFile == 4 && fromRank == 0 && toFile == 7 && toRank == 0 { // e1h1
		toFile, toRank = 6, 0
	} else if fromFile == 4 && fromRank == 0 && toFile == 0 && toRank == 0 { // e1a1
		toFile, toRank = 2, 0
	} else if fromFile == 4 && fromRank == 7 && toFile == 7 && toRank == 7 { // e8h8
		toFile, toRank = 6, 7
	} else if fromFile == 4 && fromRank == 7 && toFile == 0 && toRank == 7 { // e8a8
		toFile, toRank = 2, 7
	}

	move := squareToString(fromFile, fromRank) + squareToString(toFile, toRank)
	if promo != 0 {
		move += promoToChar(promo)
	}
	return move
}

func squareToString(file, rank int) string {
	return string(rune('a'+file)) + string(rune('1'+rank))
}

func promoToChar(p int) string {
	switch p {
	case 1:
		return "n"
	case 2:
...
func polyglotKey(pos *chess.Position) uint64 {
	...
	for sq, piece := range board.SquareMap() {
		idx, ok := pieceIndex(piece)
		if !ok {
			continue
		}
		squareIndex := int(sq)
		key ^= polyglotRandom[64*idx+squareIndex]
	}
	...
	if ep := pos.EnPassantSquare(); ep != chess.NoSquare {
		...
		if pos.Turn() == chess.White {
			if hasPawnToCapture(board, chess.White, file, rank-1) {
				key ^= polyglotRandom[772+file]
			}
		} else {
			if hasPawnToCapture(board, chess.Black, file, rank+1) {
				key ^= polyglotRandom[772+file]
			}
		}
	}
...
func hasPawnToCapture(board *chess.Board, color chess.Color, file, rank int) bool {
	for _, df := range []int{-1, 1} {
		f := file + df
		if f < 0 || f > 7 || rank < 0 || rank > 7 {
			continue
		}
		sq := chess.NewSquare(chess.File(f), chess.Rank(rank))
		p := board.Piece(sq)
		if p == chess.NoPiece {
			continue
		}
		if p.Color() == color && p.Type() == chess.Pawn {
			return true
		}
	}
	return false
...
func pieceIndex(p chess.Piece) (int, bool) {
	if p == chess.NoPiece {
		return 0, false
	}
