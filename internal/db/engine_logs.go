...
func (s *Store) InsertEngineLogs(ctx context.Context, gameID int64, logs []EngineLog) error {
	...
	stmt, err := tx.PrepareContext(ctx, `
		INSERT OR REPLACE INTO engine_logs (game_id, ply, engine_id, log)
		VALUES (?, ?, ?, ?)
	`)
	if err != nil {
		return err
	...
	for _, entry := range logs {
		if _, err = stmt.ExecContext(ctx, gameID, entry.Ply, entry.EngineID, entry.Log); err != nil {
			return err
		}
	}
...
func (s *Store) ListEngineLogsByGame(ctx context.Context, gameID int64) ([]EngineLog, error) {
	...
	err := s.db.SelectContext(ctx, &out, `
		SELECT game_id, ply, engine_id, log
		FROM engine_logs
		WHERE game_id = ?
